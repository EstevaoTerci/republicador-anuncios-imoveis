...
5. **Quartos e banheiros.** Preencha com `chrome_fill_or_select` os inputs dentro dos rótulos "Número de quartos" e "Número de banheiros" com os números da ficha (XPath: `//label[contains(., "Número de quartos")]//input`). A ficha já traz o padrão (1) quando o site não informa. Rode a conferência: `campos` deve mostrar os dois valores. Sem eles o Facebook gera o título só como "Casa"/"Apartamento" e o anúncio fica incompleto.
...
8. **Fotos, uma por chamada, conferindo o contador a cada envio.** O contador é o texto "N/50" da seção Fotos; leia-o com:
   ```js
   await new Promise(r => setTimeout(r, 2500));
   return ([...document.querySelectorAll('span')].map(s => s.innerText).find(t => /\d+\/50/.test(t)) || '0/50').match(/(\d+)\/50/)[1];
   ```
   Para cada caminho de `caminhosAbsolutosEmOrdem`, na ordem: (a) leia o contador; (b) chame `chrome_upload_file` com `selector: input[type="file"][accept*="image"]`, `filePath` = o caminho, `multiple: false`; (c) leia o contador de novo. Tem que ter subido **exatamente 1**. Se subiu 0, espere 3 s e leia de novo antes de repetir a foto (a chamada imediata após outra falha com "Could not find node": repita uma vez). **Se subiu 2, pare de enviar fotos**: a extensão do Chrome está sem a correção de 15/09/2026. Feche a aba sem salvar, registre no caderno de aprendizados e diga ao usuário: "a extensão do navegador está desatualizada; rode o Atualizar.ps1, feche e abra o Chrome, e tente de novo". Não tente remover as duplicadas: remover uma remove as duas e a galeria fica inconsistente. Ao final, o contador deve ser igual ao total da ficha.
9. **Localização (automática, validada em 15/09/2026).** O campo é `input[role="combobox"][aria-label=""]` (o único combobox-input do formulário além da busca do Facebook).
   1. Traga-o para a tela e preencha com `chrome_fill_or_select` (seletor CSS acima) usando o valor "Localização (digitar exatamente)" da ficha, ex.: `Mantena, MG`. Se der "not visible", rode antes `document.querySelector('input[role="combobox"][aria-label=""]').scrollIntoView({block:'center'})` com `chrome_javascript` e repita.
   2. Espere e localize a opção da cidade, trazendo-a para a tela e pegando o centro dela (troque a regex pelo valor "Localização (opção a escolher, regex)" da ficha):
      ```js
      await new Promise(r => setTimeout(r, 2500));
      const re = /^(\d{5}-\d{3}\s+)?Mantena, MG\s+Cidade$/;
      const alvo = [...document.querySelectorAll('[role="option"]')].find(o => re.test(o.innerText.replace(/\s+/g, ' ').trim()));
      if (!alvo) return { achou: false, opcoes: [...document.querySelectorAll('[role="option"]')].map(o => o.innerText.replace(/\s+/g, ' ').trim().slice(0, 60)).slice(0, 12) };
      alvo.scrollIntoView({ block: 'center' });
      await new Promise(r => setTimeout(r, 600));
      const r = alvo.getBoundingClientRect();
      return { achou: true, texto: alvo.innerText.replace(/\s+/g, ' ').trim(), x: Math.round(r.x + r.width / 2), y: Math.round(r.y + r.height / 2) };
      ```
   3. Clique **por coordenadas**: `chrome_click_element` com `coordinates: { x, y }` (sem `selector`). Cliques por seletor/XPath NÃO funcionam nesta lista (o Facebook ignora eventos sintéticos); o clique por coordenadas gera evento confiável e funciona.
   4. Confira: o `value` do input deve virar a cidade (ex.: "35290-000 Mantena, MG"), `aria-expanded` deve ser "false" e o botão "Avançar" deve estar habilitado (`aria-disabled` nulo). Se `achou: false`, a lista pode ainda estar carregando: espere 2 s e repita o passo 2; se a cidade não aparecer, apague e digite de novo. Só depois de 3 falhas peça ao usuário: "clique na opção '<Cidade, UF>' marcada como Cidade, por favor".
