...
7. Fotos: `input[type="file"][accept*="image"]` — **uma única chamada** de `chrome_upload_file` com `multiple: true` e todos os caminhos. Chamadas repetidas **acumulam** (não substituem): foi assim que as fotos saíram em dobro no teste de 15/09/2026. Depois do envio, conte as miniaturas (`img[src^="blob:"]`) e compare com a ficha antes de qualquer outra chamada. Limite: 50.
